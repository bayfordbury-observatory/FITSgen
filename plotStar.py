...
def main(skymag):
	...
	im = np.full((height, width), bg+rn+dc, dtype=float)
	...
	#for line in range(0, 100):
		...
		yc = float(parts[1])
		mag = float(parts[2])
		I =2.5*math.pow(2.5118864315,(21.69885062-mag))*exp/expin
		#I = float(parts[2])	
		#mag = float(parts[3])	
	...
	hdu.writeto(name+"_s"+str(skymag)+"_e"+str(exp)+"_sig"+str(seeing)+"_new_subp.fits")
